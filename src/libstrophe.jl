...
[The low-level bot example](@ref low-level-examples) uses a libstrophe supplied callback function that sets basic keepalive parameters ([`xmpp_sockopt_cb_keepalive`](@ref)).
