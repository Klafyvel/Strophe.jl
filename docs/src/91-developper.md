...
In particular, the Julia code is formatted using [Runic.jl](https://github.com/domluna/JuliaFormatter.jl), so please install it globally first:
...
Strophe.jl is based on [the C libstrophe library](https://github.com/strophe/libstrophe/),
which already does the testing for the core XMPP functionnalities. However, to
test the binding, we perform some integration tests. These require an XMPP server
to talk to and some users already configured. For convenience, we provide a
docker-compose file that sets up a [prosody](https://github.com/bjc/prosody) server.
We also use an external program to simulate an extermal XMPP entity: [go-sendxmpp](https://salsa.debian.org/mdosch/go-sendxmpp), which you need to install.
You need `docker` to run the image. Then, you can navigate to the `scripts/` directory
where you will find a file named `docker-compose.yml`.

First, create the `data/` directory, that will be used to store prosody's data.
...
Then, in another terminal in the same folder, create two test users.
...
!!! note
    As long as you do not delete the `data/` directory, you won't need to create
    these users anymore.

To run some integration tests, you need julia to run with several threads. You
can either `export JULIA_NUM_THREADS=auto` in your terminal, or start julia with
`JULIA_NUM_THREADS=auto julia` in the repository folder. Thenm activate the
environment, and run `test`:
...
- Follow [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/).
  This means your commit message should look something like:
...
!!! tip "Atomic git commits"
    Try to create "atomic git commits" (recommended reading: [The Utopic Git History](https://web.archive.org/web/20230702073048/https://blog.esciencecenter.nl/the-utopic-git-history-d44b81c09593)).

- Make sure the tests pass.
- Make sure the pre-commit tests pass.
- Make sure you completed `CHANGELOG.md`, see [Keep a Changelog](https://keepachangelog.com/en/1.0.0/#how)
